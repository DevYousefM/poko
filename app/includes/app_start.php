...
function getTitle()
{
    ...
    if ($zon['page'][0] == 'autoplay') {
        ...
        echo $zon['config']['games_title'];
    } else if (isSlugCategory($zon['page'][0])) { // for category
        $category_slug = trim(urldecode($zon['page'][0]));
        $data = dataBy("SELECT * FROM zon_category WHERE slug='$category_slug'")[0];
        ...
        $game_slug = $zon['page'][1];
        $game = dataBy("SELECT * FROM zon_games WHERE game_slug='$game_slug'")[0];
...
function isSlugCategory($slug)
{
    if (num_rows(T_ZON_CATEGORY, "slug='$slug'")) {
        return true;
    } else {
        return false;
    }
